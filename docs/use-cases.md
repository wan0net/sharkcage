...
layout: doc
title: Use Cases
description: 17 concrete scenarios with exact CLI commands, Nomad API calls, and expected outcomes.
---
...
4. `run-agent.sh` does the following:
   ...
   - Generates an OpenShell sandbox policy from the dispatch metadata, scoping filesystem access to the project workspace and `/tmp`, and network access to the required hosts (LLM API endpoint, GitHub). Wraps the agent process in `openshell-sandbox` with this policy.
...
3. `run-agent.sh` reads `NOMAD_META_mode=review` and configures the coding agent accordingly:
   ...
   - The sandbox policy places the project workspace in the `read_only` Landlock allowlist instead of `read_write`. The agent physically cannot write to the project directory -- this is enforced at the kernel level, not just by instruction.
...
3. On `yeet-01`, `run-agent.sh` sees `NOMAD_META_device=yubikey`. Before launching the agent, it:
   ...
   - The sandbox policy includes `/dev/yubikey-1` in the Landlock `read_write` allowlist. The agent can access this specific device but no other `/dev/` paths.
...
Risky operations never happen silently. The user maintains control over changes to CI, deployment configs, secrets, and other sensitive files. The approval mechanism lives entirely in `run-agent.sh` and Nomad Variables -- no custom API needed. Additionally, the sandbox's network policy means the agent can only reach declared hosts (LLM API, GitHub) -- it cannot exfiltrate data to arbitrary endpoints even if it attempted to.
...
## 17. Sandbox Policy Inspection

### Scenario

A task fails because the agent can't reach the npm registry. You want to see what sandbox policy the task is running under to diagnose the issue.

### Command

```bash
$ yeet policy dispatch-a1b2c3d4
```

### Under the Hood

1. The `yeet policy` command reads the generated sandbox policy YAML from the Nomad allocation filesystem (via `GET /v1/client/fs/cat/<alloc-id>?path=local/sandbox-policy.yaml`) or from Nomad Variables at `yeet/policies/<dispatch-id>`.
2. The policy was generated by `run-agent.sh` from the dispatch metadata before wrapping the agent in `openshell-sandbox`.

### Result

```bash
$ yeet policy dispatch-a1b2c3d4
Sandbox policy for dispatch-a1b2c3d4 (peer6, opencode)
...
Seccomp: default profile (no raw sockets, no mount, no ptrace)
```

The missing npm registry host (`registry.npmjs.org`) is immediately visible. The user can add it to the project's allowed hosts config and re-run the task.

---

## Summary

These seventeen use cases cover the primary interaction patterns with yeet:
...
| 17 | Sandbox policy inspection | `yeet policy` shows filesystem, network, and device allowlists for debugging and auditing |
